...
class ResizeInstanceMask : public ResizeBBox {
 public:
  explicit ResizeInstanceMask(const Value& cfg) : ResizeBBox(cfg) {
    if (cfg.contains("params")) {
      ...
      is_origin_mask_ = cfg["params"].value("is_origin_mask", is_origin_mask_);
 ...
 protected:
  Result<void> ProcessMasks(Detections& result, Tensor d_mask, Tensor cpu_dets, int img_w,
    ...
    if (is_rcnn_) {  // mask r-cnn
    ...
    } else if (is_origin_mask_) { // mmab mask
      ...
      // std::cout << "第一次mat克隆地址" << (void *)mask_mat.data << std::endl;
      Tensor mask_t = cpu::CVMat2Tensor(mask_mat);
      // std::cout << "第一次tensor转换地址" << mask_t.data<void>() << std::endl;
  ...
  Mat ThresholdMask(const Tensor& h_mask) const {
    // std::cout << "第二次tensor引用地址" << h_mask.data<void>() << std::endl;
    cv::Mat warped_mat = cpu::Tensor2CVMat(h_mask);
    // std::cout << "第二次mat地址" << (void *)warped_mat.data<< std::endl;
    if (is_origin_mask_){
      warped_mat = warped_mat.clone();
      // std::cout << "第三次mat克隆地址" << (void *)warped_mat.data<< std::endl;
      // std::cout << "--------------------------------------" << std::endl;
      // use shape to convert float, 在python中重新解释成float类型
      return {warped_mat.rows, warped_mat.cols*int(sizeof(float)), PixelFormat::kGRAYSCALE, DataType::kINT8,
